...
from ImportDB.util import http_get, http_post, setup_django_rentime_depend, generate_write_jwt
setup_django_rentime_depend()

datapoints_url = "http://localhost:8000/DataTables/13/DataPoints/"
datatable_url = "http://localhost:8000/DataTables/?application=7"
dataset_url = "http://localhost:8000/DataSets/7/"
...
for point in http_get(datapoints_url):
    ...
    for table in http_get(datatable_url):
        ...
        if table['datapoint_strategy'] == 'Station':
            nears_point_url = table_url + 'DataPoints/Nearest/lat/%s/lng/%s/' % (point['lat'], point['lng'])
            try:
