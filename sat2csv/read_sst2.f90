...
contains
  ...
  real, dimension(:,:),pointer :: values,rsvals
  real, dimension(:),pointer:: lats,lons,rslat,rslon
  character(len=200) :: long_name, frmt, cmd
  character(len=100):: varname, cbar, workdir, output
  real :: miss
  integer :: rgb(4,256), gain, offset
  integer :: minlat,maxlat,minlon,maxlon,nx,ny
  ...
  nx=0;ny=0
  minlon=116;maxlon=126
  minlat=19;maxlat=29
  do i = 1 , ix
     if(lons(i)<maxlon.and.lons(i)>minlon)nx=nx+1
     if(lons(i)>minlon.and.lons(i-1)<minlon)xmin=i
     if(lons(i)<maxlon.and.lons(i+1)>maxlon)xmax=i
  enddo
  do i = 1 , iy
     if(lats(i)<maxlat.and.lats(i)>minlat)ny=ny+1
     if(lats(i)>minlat.and.lats(i-1)<minlat)ymin=i
     if(lats(i)<maxlat.and.lats(i+1)>maxlat)ymax=i
  enddo
  allocate(rslon(nx))
  allocate(rslat(ny))
  allocate(rsvals(nx,ny))
  rslon=lons(xmin:xmax)
  rslat=lats(ymin:ymax)
  rsvals(1:nx,1:ny)=values(xmin:xmax,ymin:ymax)
  
  where(rsvals==miss)rsvals=-99999.0
  miss=-99999.0
  ...
  open(90,file=output,status="unknown")
      ...
      enddo
99    k=i-1
   write(90,'(3(G0.5,:,","),(f7.3,","),7(G0.6,:,","))')nx,ny,"b",rslon(1),rslat(1), &
            rslon(nx),rslat(ny),"float32",miss,miss,"EPSG:4326"
  ...
  !      abs(lats(iy)-lats(1))/iy,"float32",miss,miss
   write(90,'(a,G0.5,a,G0.5,a)')"linearTransform(",gain,",",offset,")"
    ...
    if(rgb(4,k)>rgb(4,1))then
      ...
      do jj=1,ny-1
         write(90,frmt) (rsvals(ii,jj),ii=1,nx)
      end do
      write(90,frmt,advance="no") (rsvals(ii,ny),ii=1,nx)
      DEALLOCATE(values)
      close(80)
      close(90)
