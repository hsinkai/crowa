      ...
      CHARACTER(LEN=200) :: output, list(10), workdir, cbar
      CHARACTER(len=8) :: dd
      CHARACTER(len=4) :: tt
      CHARACTER(len=2) :: bb
     ...
     ! command line
      ...
      enddo
      !open(50,file="ir1.dat",form='unformatted',access='direct',recl=nx*ny*nz*4,status='unknown')
      do i = 1 , 10
         do j = 1 , ny
      ...
      ystart=-5.
      resol=0.02
      xend=xstart+(np-1)*resol
      yend=ystart+(np-1)*resol
      gain=1.0;offset=0.0
      allocate(buf(np,np))
      do i = 1 , np
        do j = 1 , np
           rlat=ystart+real(i-1)*resol
           rlon=xstart+real(j-1)*resol
           call geocoord2pixcoord(rlat,rlon,HisdHeader1%proj%coff,HisdHeader1%proj%loff,ll,lll,HisdHeader1)
           buf(j,np+1-i)=buf2(ll,lll)
           !call pixcoord2geocoord(ll,lll,rlat1,rlon1,HisdHeader1)
           !if(rlat>24.9.and.rlat<25.and.rlon>121.4.and.rlon<121.5)print *,ll,lll,np+1-i,j,rlat,rlon
        enddo
      enddo
      !open(50,file="ir1.dat",form='unformatted',access='direct',recl=np*np*4,status='unknown')
      !write(50,rec=1)buf
      ...
      enddo
99    k=i-1
      write(90,'(3(G0.5,:,","),4(f0.2,","),4(G0.5,:,","))')np,np,"T",xstart,yend, &
            xend,ystart,"int16",-9999,-9999,"EPSG:4326"
      ...
      write(frmt,'(a1,i4,a13)')"(",np+1,'(G0.5,:,","))'
      do jj=1,np-1
         write(90,frmt) (nint(buf(ii,jj)),ii=1,np)
      end do
      write(90,frmt,advance="no") (nint(buf(ii,np)),ii=1,np)
      DEALLOCATE(buf2,buf)
      close(80)
      close(90)
