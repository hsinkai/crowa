      ...
      integer(i_byte),allocatable,dimension(:,:) :: buf2
      integer(i_short),allocatable,dimension(:,:) :: buf
      CHARACTER(len=100) :: input1, input2, frmt
     ...
     ! command line
      ...
      !write(50,rec=1)buf
      open(60,file="lookup.tab",status="old")
      do i = 1 , 256
         read(60,*)rgb(4,i),rgb(1,i),rgb(2,i),rgb(3,i) 
      enddo
      write(6,'(3(G0.5,:,","),4(f0.2,","),4(G0.5,:,","))')np,np,"T",xstart,ystart,xstart+50,ystart+50,"int16",-9999,-9999,"EPSG:4326"
      write(6,'(a,f6.4,a,f6.4,a)')"LinearTransform(",gain,",",offset,")"
      if(rgb(4,256)>rgb(4,1))then
         write(6,'(9999(G0.5,:,","))')rgb(1:4,1:256)
      else
         write(6,'(9999(G0.5,:,","))')((rgb(i,j),i=1,4),j=256,1,-1)
      endif
      where (buf < 0) buf=buf+256 
      write(frmt,'(a1,i4,a13)')"(",np+1,'(G0.2,:,","))'
      do jj=1,np-1
         write(6,frmt) (buf(ii,jj),ii=1,np)
      end do
      write(6,frmt,advance="no") (buf(ii,np),ii=1,np)
      DEALLOCATE(buf2,buf)
      stop
      end
