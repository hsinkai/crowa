...
contains
  ...
  ! command line
   if ( k /= 5 ) then
        ...
        print *, " ./read_ocm3.exe <nc filename> <nc filename> <color bar> <working directory> <output file>"
        print *, " EXIT! "
        stop
   end if
...
!!
  ...
  call outpnt(repo,nx,ny,minlon,minlat,dx,dy,cbar,output,miss)
...
99    k=i-1
  !write(6,'(3(G0.5,:,","),4(f0.2,","),4(G0.5,:,","))')ix,iy,"T",lons(1),lats(1),lons(ix),lats(iy),"float32",miss,miss,"EPSG:4326"
    ...
    if(rgb(4,k)>rgb(4,1))then
      ...
      write(frmt,'(a1,i4,a13)')"(",ny+1,'(f0.2,:,","))'
      do ii=1,nx-1
